        ...
        .left_paren => {
            ...
            while (true) {
                switch (try self.readNextImpl()) {
                    ...
                    .end_expr => {
                        const list = try self.vm.builder().makeList(elements.items);
...
fn buildList(self: Reader, elements: []const Val) Vm.Error!Val {
    return self.vm.builder().makeList(elements);
}

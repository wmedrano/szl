                ...
                while (pattern_idx < patterns.items.len) {
                    ...
                    if (pat == .ellipsis) {
                        ...
                        while (expr_idx < expr_list.len) {
                            ...
                            var temp_bindings = Template.Bindings.init(vm.allocator());
                            defer temp_bindings.deinit(vm.allocator());

                            if (try matchPattern(ellipsis_pattern.*, expr_list[expr_idx], &temp_bindings, vm)) {
                                try matches.append(vm.allocator(), expr_list[expr_idx]);

                                // Merge ellipsis bindings
                                try mergeEllipsisBindings(bindings, temp_bindings, vm.allocator());
                                expr_idx += 1;
                            } else {
                                break;
                            }
                    ...
                    if (!try matchPattern(pat, expr_list[expr_idx], bindings, vm)) {
                        return false;
                    }
            ...
            .list => |templates| {
                ...
                for (templates.items) |tmpl| {
                    if (tmpl == .ellipsis) {
                        ...
                        const rep_count = try ellipsis_tmpl.getEllipsisRepetitionCount(bindings);
                        ...
                        while (i < rep_count) : (i += 1) {
                            ...
                            try extractEllipsisBindings(&temp_bindings, bindings, i);

                            const expanded = try ellipsis_tmpl.expand(temp_bindings, vm);
                            try results.append(vm.allocator(), expanded);
                        }
                    } else {
                        const expanded = try tmpl.expand(bindings, vm);
                        try results.append(vm.allocator(), expanded);
                    }
                }
    ...
    fn getEllipsisRepetitionCount(
        ...
        switch (self) {
            ...
            .list => |templates| {
                ...
                for (templates.items) |tmpl| {
                    const count = try tmpl.getEllipsisRepetitionCount(bindings);
                    if (count > max_count) max_count = count;
                }
                return max_count;
            },
            .ellipsis => |e| {
                return try e.template.getEllipsisRepetitionCount(bindings);
            },
            .literal => return 0,
        }
    }
...
pub fn expand(self: SyntaxRules, vm: *Vm, usage: Val) Vm.Error!?Val {
    ...
    for (self.rules.items) |*rule| {
        ...
        if (try rule.pattern.matchPattern(usage, &bindings, vm)) {
            return try rule.template.expand(bindings, vm);
        }
    }

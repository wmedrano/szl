...
pub fn init(vm: *Vm) Vm.Error!Handle(Module) {
    ...
    const env_handle = try b.makeEnvironment(&.{
        ...
        .{ .symbol = (try b.makeStaticSymbolHandle("*")), .value = Val.initNativeProc(&number_fns.mul) },
        .{ .symbol = (try b.makeStaticSymbolHandle("/")), .value = Val.initNativeProc(&number_fns.div) },
        ...
        .{ .symbol = (try b.makeStaticSymbolHandle("finite?")), .value = Val.initNativeProc(&number_fns.finite_p) },
        .{ .symbol = (try b.makeStaticSymbolHandle("zero?")), .value = Val.initNativeProc(&number_fns.zero_p) },
        .{ .symbol = (try b.makeStaticSymbolHandle("positive?")), .value = Val.initNativeProc(&number_fns.positive_p) },
        .{ .symbol = (try b.makeStaticSymbolHandle("negative?")), .value = Val.initNativeProc(&number_fns.negative_p) },
        .{ .symbol = (try b.makeStaticSymbolHandle("odd?")), .value = Val.initNativeProc(&number_fns.odd_p) },
        .{ .symbol = (try b.makeStaticSymbolHandle("even?")), .value = Val.initNativeProc(&number_fns.even_p) },
        .{ .symbol = (try b.makeStaticSymbolHandle("max")), .value = Val.initNativeProc(&number_fns.max) },
        .{ .symbol = (try b.makeStaticSymbolHandle("min")), .value = Val.initNativeProc(&number_fns.min) },
        .{ .symbol = (try b.makeStaticSymbolHandle("abs")), .value = Val.initNativeProc(&number_fns.abs) },
        // 6.3 Booleans
